...
- `options` extend [`RequestInit`](https://github.com/nodejs/undici/blob/v5.7.0/types/fetch.d.ts#L103-L117), allowing you to specify a body, headers, etc. to apply to the request.
...
Returns an object with the [AsyncState](#asyncstate) corresponding to the execution of the function and a couple of methods to manipulate it.
...
By default, when an argument passed to the hook changes, the function will be executed again, and the cache's value for that argument will be returned immediately. This means that in the case of new arguments that haven't been used yet, the initial data will be returned.
...
An object is corresponding to the execution state of the function.
